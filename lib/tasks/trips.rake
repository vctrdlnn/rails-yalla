namespace :trips do
  ...
  desc "Force update ALL trip photos from seeds_data.json (replaces existing)"
  task force_update_photos: :environment do
    require 'json'
    ...
    puts "Force updating ALL trip photos from seeds_data.json..."
    puts "Found #{seeds_data[:trips].count} trips in JSON"
    ...
    seeds_data[:trips].each_with_index do |trip_data, idx|
      ...
      # Match by city only to be more flexible
      trip = Trip.find_by(city: trip_data[:city])
    ...
    puts "\n" + "=" * 50
    puts "Photo force update complete!"
    puts "  Updated: #{updated}"
    puts "  Skipped: #{skipped}"
    puts "  Failed: #{failed}"
    puts "=" * 50
